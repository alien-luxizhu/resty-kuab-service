...
cipher = function (_cipher)
    ...
    if C[func] then
        ...
        return { size=size, cipher=_cipher, method=C[func]() }
    else
        return nil
    end
end
...
function _M.new(self, key, salt, _cipher, _hash, hash_rounds)
	...
	-- no need to set padding mode
	-- C.EVP_CIPHER_CTX_set_padding(encrypt_ctx,5)
	-- C.EVP_CIPHER_CTX_set_padding(decrypt_ctx,5)
    ...
    local gen_key = ffi_new("unsigned char[?]", _cipherLength,key)
